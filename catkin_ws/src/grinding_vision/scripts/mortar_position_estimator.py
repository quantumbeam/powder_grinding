...
from grinding_vision import pixel_to_3d_position, transform_to_robot_frame


class MortarPositionEstimator:
    def __init__(self):
        ...
        self.realsense_position_estimator = pixel_to_3d_position.PixelTo3DPosition()
        self.pose_transformer = (
            transform_to_robot_frame.PoseTransformer()
        )  # PoseTransformerのインスタンス化

        self.realsense_position_estimator.default_depth_value = 0.216  # [m]
    ...
    def detection_callback(self, msg):
        ...
        if mortar_position is None:
            rospy.logwarn("Mortar position is None. Skipping frame.")
        else:
            # 3D座標を使用して姿勢を作成
            pose_3d = PoseStamped()
            ...
            rospy.loginfo(
                "Estimated Mortar position: {}".format(transformed_pose.pose.position)
            )
...
if __name__ == "__main__":
    ...
    detector = MortarPositionEstimator()
