...
class GripperController(object):
    def __init__(
        self, namespace="", prefix=None, timeout=5.0, attach_link="robot::wrist_3_link"
    ):
        ...
        if rospy.has_param(self.ns + node_name + "/joint"):
            ...
            rospy.logerr(
                "Couldn't find valid joints params in %s" % (self.ns + node_name)
            )
            return
        ...
        self._js_sub = rospy.Subscriber(
            "joint_states", JointState, self.joint_states_cb, queue_size=1
        )
        ...
        if attach_plugin:
            try:
                ...
                rospy.logdebug(
                    "Waiting for service: {0}".format(self.attach_srv.resolved_name)
                )
                rospy.logdebug(
                    "Waiting for service: {0}".format(self.detach_srv.resolved_name)
                )
                self.attach_srv.wait_for_service()
                self.detach_srv.wait_for_service()
            except Exception:
                rospy.logerr(
                    "Fail to load grasp plugin services. Make sure to launch the right Gazebo world!"
                )
        ...
        if not server_up:
            rospy.logerr(
                "Timed out waiting for Gripper Command"
                " Action Server to connect. Start the action server"
                " before running this node."
            )
            raise rospy.ROSException(
                "GripperCommandAction timed out: {0}".format(action_server)
            )
    ...
    def command(self, value, percentage=False, wait=True):
        ...
        if wait:
            self._client.send_goal_and_wait(
                self._goal, execute_timeout=rospy.Duration(2)
            )
        else:
...
class JointControllerBase(object):
    ...
    def __init__(self, namespace, timeout, joint_names):
        ...
        while not hasattr(self, "_joint_names"):
            ...
            if rospy.is_shutdown():
                return
        self.rate = utils.read_parameter(
            "{0}joint_state_controller/publish_rate".format(self.ns), 500
        )
...
class JointPositionController(JointControllerBase):
    ...
    def __init__(self, namespace="", timeout=1.0, joint_names=None):
        ...
        @param timeout: Time in seconds that will wait for the controller
        """
        super(JointPositionController, self).__init__(
            namespace, timeout=timeout, joint_names=joint_names
        )
        if not hasattr(self, "_joint_names"):
            raise rospy.ROSException(
                "JointPositionController timed out waiting joint_states topic: {0}".format(
                    namespace
                )
            )
        self._cmd_pub = dict()
        for joint in self._joint_names:
            ...
            self._cmd_pub[joint] = rospy.Publisher(
                "%s%s/command" % (self.ns, joint), Float64, queue_size=3
            )
        ...
        while not rospy.is_shutdown():
            if (rospy.get_time() - start_time) > timeout:
                raise rospy.ROSException(
                    "JointPositionController timed out waiting for the controller_manager: {0}".format(
                        namespace
                    )
                )
            rospy.sleep(0.01)
    ...
    def set_joint_positions(self, jnt_positions):
        ...
        if not self.valid_jnt_command(jnt_positions):
            rospy.logwarn(
                "A valid joint positions command should have %d elements"
                % (self._num_joints)
            )
            return
...
class JointTrajectoryController(JointControllerBase):
    ...
    def __init__(
        ...
        @param timeout: Time in seconds that will wait for the controller
        """
        super(JointTrajectoryController, self).__init__(
            namespace, timeout=timeout, joint_names=joint_names
        )
        ...
        if not server_up:
            ...
            raise rospy.ROSException(
                "JointTrajectoryController timed out: {0}".format(action_server)
            )
        rospy.logdebug("Successfully connected to [%s]" % action_server)
        # Get a copy of joint_names
        if not hasattr(self, "_joint_names"):
            raise rospy.ROSException(
                "JointTrajectoryController timed out waiting joint_states topic: {0}".format(
                    self.ns
                )
            )
