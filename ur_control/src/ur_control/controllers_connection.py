...
class ControllersConnection:
    def __init__(self, namespace=None):
        ...
        self.switch_service = rospy.ServiceProxy(
            prefix + "switch_controller", SwitchController
        )
        self.load_service = rospy.ServiceProxy(
            prefix + "load_controller", LoadController
        )
        self.unload_service = rospy.ServiceProxy(
            prefix + "unload_controller", UnloadController
        )
        self.list_controllers_service = rospy.ServiceProxy(
            prefix + "list_controllers", ListControllers
        )
    ...
    def unload_controllers(self, controllers_list):
        try:
            ...
            for controller in controllers_list:
                ...
                rospy.loginfo(
                    "Unloading controller %s. Result=%s" % (controller, result)
                )
                if result:
    ...
    def switch_controllers(self, controllers_on, controllers_off, strictness=1):
        ...
        if rospy.has_param("use_gazebo_sim"):
            controllers_on = [
                self.ns + "/" + controller for controller in controllers_on
            ]
            controllers_off = [
                self.ns + "/" + controller for controller in controllers_off
            ]
